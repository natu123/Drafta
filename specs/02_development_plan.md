...
| 3-3 | Stripe実装 | プラン管理（Free 200MB / Pro 10GB）、追加ストレージ購入（+10GB/$2） |
| 3-4 | 運用監視実装 | 保存状態UI（保存中/保存済み/保存失敗）、失敗率・遅延P95の計測 |
...
- Phase 4-5は並行作業可能
- 将来的な検討事項: オフライン対応
- 料金・容量の確定値は `specs/03_specification.md` と `specs/04_cost_analysis.md` を正本として参照
