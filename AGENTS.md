...
- Item Actions: Trayのメニューに名前変更と復元可能な削除をまとめる。PCのhover・focus時に表示しタッチ端末では常時表示する。Memo一覧にはメニューを設置せず日付を表示する。Memoの改名は編集画面のタイトルで行い、削除ボタンで復元可能な削除後に一覧へ戻る。Inboxと保護メモは変更・削除対象外。
