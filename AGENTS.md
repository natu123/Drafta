...
- Samples: 初期サンプル9件はsampleKeyで識別し、表示言語に追従する。原文テンプレートと翻訳データから描画し、通常のメモを書き換えない。編集可能なサンプルのタイトル・本文を編集したら表示中の訳文を保持してsampleKeyを外し、以後は翻訳しない。ID・保護状態・日時・アイコンは維持する。Inbox固定解除や空のプロファイル作成は別機能として扱う。
...
- Item Actions: Trayのメニューに名前変更と復元可能な削除をまとめる。PCのhover・focus時に表示しタッチ端末では常時表示する。Memo一覧には日付とピン操作を表示する。改名は編集画面のタイトルで行い、タイトル行の削除ボタンで復元可能な削除後に一覧へ戻る。InboxのTray固定は維持する。
