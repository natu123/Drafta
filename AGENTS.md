...
- Sample Notes: WelcomeとQuick Referenceも通常メモとして編集・改名・アイコン変更・削除できる。固定ノート専用表示は設置しない。
- Samples: 初期サンプル9件はsampleKeyで識別し、表示言語に追従する。原文テンプレートと翻訳データから描画し、通常のメモを書き換えない。編集可能なサンプルのタイトル・本文を編集したら表示中の訳文を保持してsampleKeyを外し、以後は翻訳しない。ID・保護状態・日時・アイコンは維持する。空のプロファイル作成は別機能として扱う。
- Layout: 3カラム（左サイドバー / 中央ノートリスト / 右エディタ）、比率 1.8:3.5:6.7
- Editor Modes: Rich（TipTap）/ Plain（textarea、Markdown）切り替え対応
- Rich Input: 引用・箇条書き・番号付きリスト・チェックリスト・見出し1〜3・コードフェンス・区切り線と、太字・斜体・取り消し線・インラインコード・色タグを自動変換する。Markdown表はヘッダー行と区切り行の後にEnterで変換する。タイトル・コードブロック・Plain表示・保護メモでは自動変換しない。貼り付け処理は従来どおりとする。
- Editor Commands: 本文の段落行頭の`/`で挿入メニューを開き、検索・矢印キー・Enter・クリックで選択、Escで閉じる。段落・見出し1〜3・各種リスト・引用・コード・区切り線・表を用意する。選択時の書式メニューは太字・斜体・取り消し線・インラインコードとし、Ctrl/Cmdショートカットを表示する。
...
- Creation: Add Tray / Add Memoは一覧内の入力欄を開き、Enterまたは確定ボタンで作成する。IME変換中は確定せず、Escまたはキャンセルでデータを残さず取り消す。空欄の明示確定は無題として作成する。選択と一覧を維持し、作成項目は選んだときに開く。
- Item Actions: Trayのメニューに名前変更と復元可能な削除をまとめる。PCのhover・focus時に表示しタッチ端末では常時表示する。Memo一覧には日付とピン操作を表示する。改名は編集画面のタイトルで行い、タイトル行の削除ボタンで復元可能な削除後に一覧へ戻る。Inboxも通常Trayとして改名・削除・並べ替え可能とし、初期状態のみピン留めする。日本語の復元一覧の表示名は「復元」とする。
- Tray Pins: Tray一覧の先頭にピン留めTrayをまとめ、見出し1行へ折りたためる。通常Trayは表示したまま、折りたたみは現在のセッションで保持する。各領域内の手動順を保持する。最後のTrayを削除したら復元一覧へ移り、Tray再作成時に選択する。バックアップはInboxなし・空の構成とTrayのisPinnedを許容する。
- Pins: 各Trayの先頭にピン留めメモを表示する。同じ一覧内で見出し1行へ折りたため、通常メモは表示したままにする。折りたたみ状態はTrayごとに現在のセッションで保持する。解除時は通常リストへ戻る。各領域内の手動順を保持し、ピンアイコンのみゴールドとする。
- Editor Layout: ノートアイコンはタイトル左、削除はタイトル右端。PCのツールバーは最大2段で常時表示する。SPの通常時は戻る操作・7色パレット・ツールボタンを1列に表示し、その他のツールはボタンで展開する。
- Search: ノート内検索はタイトル・本文・コード内を対象に一時的な強調表示、件数、前後移動を提供する。検索装飾を保存しない。全体検索のメモ結果には更新日を表示する。
- Page Title: アプリのブラウザータイトルは「メモの表示タイトル - Drafta」。未選択時はDrafta。ランディングのメタ情報は維持する。
...
UI 状態カラー:
- **Primary Blue** — アクティブノート・アクティブグループの背景と枠線（opacity付きで使用）
- **Gold #C49547** — ピン留め状態のアイコン表示（背景は通常メモと共通）
